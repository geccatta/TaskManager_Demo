...
public class TaskData{
    ...
    public static Set<Task> getTasks(String assignee){
        ...
        String user = ("ann,bob,carol".contains(assignee.toLowerCase())) ? assignee : null;
        String selectedList = switch (assignee.toLowerCase()){
            case "ann" -> annTasks;
            case "bob" -> bobTasks;
        ...
        for(String taskData : selectedList.split("\n")){
            ...
            Status status = (data.length <= 3) ? Status.NOT_ASSIGNED : Status.valueOf(data[3].toUpperCase().replaceAll(" ", "_"));
